...
void trajectory_generator::compute_circle_trj(trj_generator_input_msg& in, trj_generator_output_msg& out)
{
    out.trj.clear();
    ...
    std::cout<<"to ("<<in.displacement.p.x()<<' '<<in.displacement.p.y()<<' '<<in.displacement.p.z()<<' '<<ro_<<' '<<pi_<<' '<<ya_<<") in "<<in.time<<" [s], R: "<<in.radius<<
               " [m], angle: "<<in.center_angle<<" [rad], left: "<<in.left<<std::endl;
...
//     std::cout<<"(circle ";
    ...
    int index_1 = (2*(in.hand)+(!in.hand));
    
    int index_2 = (2*(!in.hand)+in.hand);
    
    for(double t=0;t<=in.time;t=t+0.01)
    {
        //CircleAngle = 20.0*Xf1/(2.0*Tf*Tf*Tf)*t*t*t                   + (-30.0*Xf1)/(2.0*Tf*Tf*Tf*Tf)*t*t*t*t              + (12.0*Xf1)/(2.0*Tf*Tf*Tf*Tf*Tf)*t*t*t*t*t;
	circle_angle =  20.0*Xf1/(2.0*pow(in.time,3))*pow(t,3) + (-30.0*Xf1)/(2.0*pow(in.time,4))*pow(t,4) + (12.0*Xf1)/(2.0*pow(in.time,5))*pow(t,5);
	circle_angle = circle_angle - atan2(dis_vector_p.data[index_2],dis_vector_p.data[0]);

	temp_vector_p.data[index_1] = start_vector_p.data[index_1] + 20.0*(dis_vector_p.data[index_1])/(2.0*pow(in.time,3))*pow(t,3) + (-30.0*dis_vector_p.data[index_1])/(2.0*pow(in.time,4))*pow(t,4) + (12.0*dis_vector_p.data[index_1])/(2.0*pow(in.time,5))*pow(t,5);
        ...
        if(!in.left) //RIGHT
	{
	    ...
	    temp_vector_p.data[index_2]=radius*sin(circle_angle)+in.hand*mean_y+(!in.hand)*mean_z;
        }
        else //LEFT
	{
	    //Xd_v(0) = -Radius*cos(CircleAngle) ;
